...
###########################################################
# ISOLATION MIGRATION MODEL ABPG and 2 others lineages split
# Recent split between PG and AB (SIC) -  asymetric migration
...
for jj in range(0,10):
    ...
    # N1,N2,N3,T2,T1,m12, m21, m13, m31, m23, m32
    ...
    fout = open('3_pop_SIC_with_Asym_MIG.xls','a')
    fout.write('1st_anneal_optimization\t3pops_SIC_M\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
...
for jj in range(0, 5):
    ...
    fout = open('3_pop_SIC_with_Asym_MIG.xls','a')
    fout.write('2nd_otimization\t3pops_SIC_M\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
    fout.close()
