...
def SIA(params, ns, pts):
    N1,N2,N3,T2,T1 = params
    xx = dadi.Numerics.default_grid(pts)
    phi = dadi.PhiManip.phi_1D(xx)
    phi = dadi.PhiManip.phi_1D_to_2D(xx,phi)
    phi = dadi.Integration.two_pops(phi, xx, T2, nu1=N1, nu2=N2, m12=0., m21=0.)
    ## split between PG and ABPG - AB comes directly from ancestral
    phi = dadi.PhiManip.phi_2D_to_3D_split_2(xx, phi)
    ## allow drift and no migration to occur along these branches
    phi = dadi.Integration.three_pops(phi, xx, T1,
                                       nu1=N1, nu2=N2, nu3=N3,
                                       m12=0.0, m13=0,
                                       m21=0.0, m23=0,
                                       m31=0, m32=0)
    fs = dadi.Spectrum.from_phi(phi,ns,(xx,xx,xx),pop_ids=['AB', 'PG', 'ABPG'])
    return fs
...
for jj in range(0,10):
    ...
    params =      [1, 1, 1, 0.1, 0.1]
    upper_bound = [10, 10, 10, 1, 1]
    ...
    fout = open('3_SIA.xls','a')
    fout.write('1st_anneal_optimization\t3pops_SIA\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
...
for jj in range(0, 5):
    ...
    fout = open('3_SIA.xls','a')
    fout.write('2nd_otimization\t3pops_SIA\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
    fout.close()
