...
# Recent split between PG and AB (SIC) - no migration
#
#         |
#         |
#     --------
...
for jj in range(0,10):
    ...
    fout = open('3_SIC.xls','a')
    fout.write('1st_anneal_optimization\t3pops_SIC\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
...
for jj in range(0, 5):
    ...
    fout = open('3_SIC.xls','a')
    fout.write('2nd_otimization\t3pops_SIC\t' + str(ll_opt) + '\t' + str(theta) + '\t' + str(popt).replace('\n','') + '\t' + str(AIC) + '\n')
    fout.close()
